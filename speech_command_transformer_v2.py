...
mlflow.set_experiment("Speech_Command_Transformer_Exp")
...
def run_training_and_evaluation(model, train_loader, test_loader, criterion, optimizer, scheduler, epochs):
    ...
    for epoch in range(epochs):
        ...
        mlflow.log_metric("test_precision", precision, step=epoch+1)
        mlflow.log_metric("test_recall", recall, step=epoch+1)
        mlflow.log_metric("test_f1", f1, step=epoch+1)
...
def main():
    ...
    with mlflow.start_run():
        ...
        mlflow.pytorch.log_model(model, "speech_transformer_model")
